class RSSManager {  
  constructor() {
    ...
    if (window.location.port === '80' || window.location.host.includes(':80')) {
      this.apiBaseUrl = `${this.baseUrl}/api`;
    } else {
      this.apiBaseUrl = `http://localhost:80/api`;
    }
  }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error('Backend server not available. Please ensure the server is running on port 80.');
        }
      ...
      if (error.message.includes('fetch') || error.name === 'TypeError') {
        this.showNotification('Backend server not available. Please ensure the server is running on port 80.', 'error');
      } else {
