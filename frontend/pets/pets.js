...
async function fetchPets() {
  try {
    if (!petService) {
      if (window.PetService) {
        petService = new window.PetService();
      } else {
...
function renderPets(pets, containerId = 'pets-grid') {
...
function createPetCard(pet) {
  if (!window.CardRenderer) {
    console.warn('CardRenderer not available yet, creating fallback card');
    const fallbackCard = document.createElement('div');
    fallbackCard.className = 'pet-card loading';
    fallbackCard.innerHTML = '<div class="placeholder-content">Loading...</div>';
    fallbackCard.setAttribute('data-pet-id', pet.id);
    return fallbackCard;
  }
...
function showPetLoadError(error, containerId = 'pets-grid') {
  const container = document.getElementById(containerId);
  if (!container) return;
...
function showPetPlaceholders(containerId = 'pets-grid', count = 6) {
  const container = document.getElementById(containerId);
  if (!container) return;
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (window.CardRenderer && window.CardRenderer.createPlaceholderCard) {
      placeholder.innerHTML = window.CardRenderer.createPlaceholderCard('pet');
    } else {
      console.warn('CardRenderer not available, using fallback placeholder');
      placeholder.className = 'pet-card placeholder card-placeholder';
...
function initializeServices() {
  if (window.PetService && !petService) {
    petService = new window.PetService();
