...
class UserController {
  ...
  async createUser(req, res) {
    try {
      ...
      const verificationLink = `${process.env.BASE_URL || `${process.env.API_PROTOCOL || 'http'}://${process.env.API_HOST || 'localhost'}:${process.env.API_PORT || 80}`}/api/users/verify-email?token=${emailToken}`;
      const mailOptions = {
